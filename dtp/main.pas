...
{    Version 1.07, 21th Sep 23 - paweld:                                        }
...
uses
  ...
  StrUtils, FileUtil, LCLIntf, ComCtrls, HtmlView, HtmlGlobals, Graphics,
  fpjson, jsonparser, base64, LPDNetU, BGRABitmap, BGRABitmapTypes, BioWS;
...
function GetImgUrlToBase64Png(Url: string; InlineImg: boolean;
  newWidth: integer; newHeight: integer; Proportional: boolean): string;
function GetAndMergeImages(baseUrl, pointsUrl: string; resultFile: string;
  newWidth: integer; newHeight: integer; Proportional: boolean): string;
...
begin
  Result := False;
  if (ExtractFilePath(AsName) <> '') and not DirectoryExists(
    ExtractFilePath(AsName)) and not ForceDirectories(ExtractFilePath(AsName)) then
    Exit;
...
begin
  ...
  begin
    ...
    begin
      if FGBIFSearch.status <> 'accepted' then
        status := ' (' + FGBIFSearch.status + ' of <i>' + FGBIFSearch.valid_name +
          '</i>' + ' ' + FGBIFSearch.authorship + ')'
      else
        status := ' (' + FGBIFSearch.status + ')';
    end;
    taxon_list := FGBIFSearch.kingdom + '; ' + FGBIFSearch.phylum +
      '; ' + FGBIFSearch.classe + '; ' + FGBIFSearch.order + '; ' + FGBIFSearch.family;
    Fresults.Add('<h2><i>' + Fname + '</i>' + ' ' + FGBIFSearch.authorship +
      status + '</h2>');
  ...
  if not Terminated then
  begin
    urlWiki := 'http://en.wikipedia.org/wiki/' + StringReplace(Fname,
      ' ', '_', [rfReplaceAll]);
  ...
  begin
    ...
    begin
      ...
      Fresults.Add('Sequences: ' + '<a href="' + urlNuc + '">' +
        IntToStr(FNCBISearch.nucNum) + '</a> nucleotide, ' + '<a href="' +
        urlProt + '">' + IntToStr(FNCBISearch.protNum) + '</a> protein');
    end;
  end;
  if not Terminated then
  begin
    ...
    begin
      ...
      begin
        Fresults.Add('<li><a href="' + linkOut.Names[i] + '">' +
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Fresults.Add(refUrl + '<img src="' + GetImgUrlToBase64Png(imgUrl,
          True, 94, 145, True) + '" width=94 height=145 border=1></a>');
      end;
    end;
  end;
  if not Terminated then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Fresults.Add('<b><a href="http://dx.doi.org/doi:' +
          pubs.ValueFromIndex[i] + '">' + pubs.Names[i] + '</a></b><br>');
        if Terminated then
          break;
      end;
    end;
...
begin
  ...
  begin
    ...
    logo := '<img src="data:image/png;base64,' + EncodeStringBase64(ss.DataString) +
      '" height="73" width="385">';
    ss.Free;
  end;
...
begin
  ...
  if FileExists(AppPath + '/static/index.htm') then
    ...
    HtmlViewer.LoadFromString(
      '<html><body bgcolor="white"><br><br><br><center><font size=7 color="green">e-Species</font></center></body></html>');
end;
...
begin
  ...
  begin
    MessageDlg('Error', 'Please enter a binomial specific epithet into the text box.' +
      sLineBreak + sLineBreak + 'Remember: Only *species* are true natural entities!',
      mtError, [mbOK], 0);
    SearchComboBox.Text := '';
  end
  else
